...
export default function TabsClipPath() {
  ...
  return (
    <div className="relative mx-auto flex w-fit flex-col items-center pt-16">
        ...
        {TABS.map((tab) => (
          <li key={tab.name}>
            <button
              ...
              className="flex h-[34px] items-center gap-2 rounded-full px-4 text-sm font-medium text-black no-underline"
            >
              {tab.icon}
              {tab.name}
            </button>
      ...
      <div
        aria-hidden
        className="absolute z-10 w-full overflow-hidden transition-[clip-path] duration-[250ms] ease-in-out [clip-path:inset(0px_75%_0px_0%_round_17px)]"
        ref={containerRef}
      >
        <ul className="relative flex w-full cursor-pointer justify-center gap-2 bg-[#2090FF]">
          {TABS.map((tab) => (
            <li key={tab.name}>
              <button
                data-tab={tab.name}
                ...
                className="flex h-[34px] items-center gap-2 rounded-full px-4 text-sm font-medium text-white no-underline"
                tabIndex={-1}
              >
                {tab.icon}
                {tab.name}
